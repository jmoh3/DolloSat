...
from utils import get_matrix_info, parse_filename, get_num_solutions_appmc, parse_allowed_losses
...
def get_info(infile, directory, num_samples, timeout, approxMC, parse_losses):
    ...
    if parse_losses:
        ...
        row_info['use_loss_info'] = True
    else:
        allowed_losses = None
        row_info['use_loss_info'] = False

    start = time.time()
    row_info['num_variables'], row_info['num_clauses'] = get_cnf(full_filename, cnf_filename, cell_clusters,
                                                                mutation_clusters, allowed_losses, expected_fn, expected_fp, True)
...
def write_row(row_info, file, metrics, ofile):
    if row_info:
        row = f'{file}'
...
def generate_info(files, directory, outfilename, num_samples, timeout, approxMC):
    metrics = ['filename', 'm', 'n', 'num_cell_clusters', 'num_mutation_clusters',
                'num_variables', 'num_clauses', 'formula_gen_time', 'use_loss_info', 
                'unigen_time', 'num_samples', 'num_solutions']
    ...
    for file in sorted_files:
        row_info_no_losses = get_info(file, directory, num_samples, timeout, approxMC, False)
        write_row(row_info_no_losses, file, metrics, ofile)

        row_info_with_losses = get_info(file, directory, num_samples, timeout, approxMC, True)
        write_row(row_info_with_losses, file, metrics, ofile)
